...
def new_candle():
    ...
    if request.method == 'POST':
        ...
        # ป้องกัน 400 Bad Request โดยใช้ .get()
        candle_file = request.files.get('candle_name')

        pic_file = None
        if candle_file and candle_file.filename != '':
            pic_file = save_image(candle_file)

        if temple and pic_file:
            ...
            flash('Please select a temple and upload an image.', 'warning')
